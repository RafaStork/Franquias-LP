...
const productionAnswer = html.match(/id="faq-answer-4"><div>([\s\S]*?)<\/div><\/div>/)[1];
for (const responsibility of ['<strong>Indústria</strong> → Tecnologia e produção', '<strong>Franqueado</strong> → Vendas, operação e montagem', '<strong>Franqueadora</strong> → Treinamento, processos e suporte']) assert(productionAnswer.includes(responsibility));
assert(productionAnswer.includes('sem precisar dominar toda a cadeia produtiva'));
